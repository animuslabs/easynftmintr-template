...
namespace atomicdata {
  ...
  uint64_t unsignedFromVarintBytes2(vector<uint8_t>::iterator& itr) {
    uint64_t number = 0;
    uint64_t multiplier = 1;
  ...
  ATTRIBUTE_MAP deserialize(const vector<const uint8_t>& data, const vector<atomicassets::FORMAT>& format_lines) {
